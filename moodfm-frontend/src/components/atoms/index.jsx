...
export function MoodBlob({ size = 280, drift = true, style = {}, geometry = 'blob', children }) {
  const id = useId();
      ...
      <div style={{ width: size, height: size, position: 'relative', ...style }}>
        <svg viewBox="0 0 200 200" width={size} height={size} style={{ display: 'block' }}>
          <defs>
            ...
            <filter id={`f-${id}`}><feGaussianBlur stdDeviation="0.6" /></filter>
          </defs>
          <path
            d="M 100,18 C 145,18 180,55 180,100 C 180,150 145,182 100,182 C 55,182 22,150 22,100 C 22,58 55,18 100,18 Z"
            fill={`url(#g1-${id})`} filter={`url(#f-${id})`}
          >
            {drift && (
              <animate attributeName="d" dur="14s" repeatCount="indefinite"
                values="M 100,18 C 145,18 180,55 180,100 C 180,150 145,182 100,182 C 55,182 22,150 22,100 C 22,58 55,18 100,18 Z;
          ...
          <path
            ...
            fill={`url(#g2-${id})`} opacity="0.7"
          >
            {drift && (
              <animateTransform attributeName="transform" type="translate"
                dur="11s" values="0,0; 8,-6; -6,8; 0,0" repeatCount="indefinite" />
            )}
          </path>
        </svg>
...
export function MoodWheel({ size = 320, geometry = 'circle', value, onChange }) {
  ...
  const startDrag = (e) => { setDrag(true); moveDrag(e); };
  const moveDrag = (e) => {
    if (!ref.current) return;
    const rect = ref.current.getBoundingClientRect();
    const cx = (e.touches ? e.touches[0].clientX : e.clientX) - rect.left;
    const cy = (e.touches ? e.touches[0].clientY : e.clientY) - rect.top;
    let x = cx / rect.width, y = cy / rect.height;
    if (geometry === 'circle') {
  ...
  return (
    <div style={{ width: size, position: 'relative', userSelect: 'none', padding: '24px 56px' }}>
      ...
      <div
        ref={ref} onMouseDown={startDrag} onTouchStart={startDrag}
        style={{ width:size, height:size, borderRadius:radius, position:'relative', cursor:drag?'grabbing':'grab', overflow:geometry==='blob'?'visible':'hidden', boxShadow:'inset 0 0 0 1px var(--rule)' }}
      >
        {geometry === 'blob' ? (
          ...
          <div style={{
            ...
            background: `radial-gradient(circle at 0% 0%, var(--mood-a) 0%, transparent 55%),
        ...
        <div style={{
          ...
          background:'var(--paper)', boxShadow:'0 2px 14px rgba(0,0,0,0.35), 0 0 0 1px var(--ink)',
        }}>
...
export function Logo({ size = 24 }) {
  return (
    <svg width={size} height={size} viewBox="0 0 32 32">
      <defs>
        <radialGradient id="logo-g" cx="30%" cy="30%">
          <stop offset="0%" stopColor="var(--mood-a)"/>
